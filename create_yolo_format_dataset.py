...
if __name__ == '__main__':
    ...
    for video_id, (video_path, anno_path) in enumerate(zip(video_paths, anno_paths)):
        ...
        while video.isOpened():
            ...
            if not flag:
                break
            cv2.imwrite(os.path.join(output_path, "train", "images", "{}_{}.jpg".format(video_id+1, counter)), frame)
            objects = [item for item in json_data["annotations"] if item["image_id"] == counter and item["category_id"] > 2]
            with open(os.path.join(output_path, "train", "labels", "{}_{}.txt".format(video_id+1, counter)), "w") as txt_file:
                for obj in objects:
                    bbox = obj["bbox"]
            ...
            #         xmax*= image_width
            #         ymax *= image_height
            #         cv2.rectangle(frame, (int(xmin), int(ymin)), (int(xmax), int(ymax)), color=(255, 0, 255), thickness=3)
